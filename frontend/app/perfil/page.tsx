...
import { Camera, Check, Loader2, Lock, LogOut, Pencil, Shield, User, X } from "lucide-react";
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-5 flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="font-serif text-base text-card-foreground">Trocar senha de acesso</h2>
...
export default function PerfilPage() {
  ...
  const acoes = (
    <div className="flex flex-col sm:flex-row gap-2">
      {usuario.is_admin && (
        <Button variant="outline" asChild className="rounded-full justify-center gap-2 flex-1">
      ...
      <Button
        ...
        className="rounded-full justify-center gap-2 flex-1"
        data-testid="button-logout-perfil"
      >
    ...
    <div className="min-h-screen bg-background text-foreground pb-24">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 flex flex-col gap-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-serif text-xl font-semibold text-foreground">Perfil</h1>
        </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6 flex flex-col items-center gap-4">
              <div className="relative">
                {usuario.foto_url ? (
                  ...
                  <div className="w-20 h-20 rounded-full bg-accent/30 border border-border flex items-center justify-center">
                ...
                <button
                  ...
                  className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-primary text-primary-foreground flex items-center justify-center border-2 border-background"
                  data-testid="button-editar-foto-perfil"
                >
                  {fotoMutation.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Camera className="w-3.5 h-3.5" />}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 w-full pt-4 border-t border-border">
                <div
                  className="flex flex-col items-center justify-center gap-1.5 border border-border rounded-xl py-4 px-2"
                  data-testid="stat-pontos"
                >
                  <span className="font-serif text-2xl text-primary">{usuario.pontos}</span>
                  <span className="mono-label text-center">Pontos acumulados</span>
                </div>
                <div
                  className="flex flex-col items-center justify-center gap-1.5 border border-border rounded-xl py-4 px-2"
                ...
                >
                  ...
                  <span className="mono-label text-center">Status cadastral</span>
                </div>
              </div>
            </div>

            <div className="hidden md:block">{acoes}</div>
          </div>

          <TrocarSenhaForm />

          <div className="md:hidden">{acoes}</div>
        </div>
      </main>
